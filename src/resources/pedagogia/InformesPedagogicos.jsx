...
import { generarReportePDF, generarReportePDFIA } from "./generarReportePDF";
import { generarInformePedagogicoAI } from "../../services/ai";
...
export const InformesPedagogicos = () => {
    ...
    const handleGenerateAI = async () => {
        if (!selectedValues.curso || !selectedValues.alumno) {
            notify("Seleccioná curso y alumno para generar el informe.", { type: "warning" });
            return;
        }
        setAiLoading(true);
        try{
            const texto = await generarInformePedagogicoAI({
                informes,
                contexto: {
                    curso: selectedValues.curso,
                    alumno: selectedValues.alumno,
                    materia: selectedValues.materia || null,
                }
            });
            await generarReportePDFIA({
                curso: selectedValues.curso,
                alumno: selectedValues.alumno,
                materia: selectedValues.materia || null,
                texto: texto || ''
            });        }catch(err){
            console.error('Error generando informe con IA:', err);
            notify(`Error generando informe con IA: ${err.message}`, { type: 'error' });
        }finally{
            setAiLoading(false);
        }
    };
    ...
    return(
        <Box>
            <Grid container spacing={2} alignItems="center">
                ...
                <Grid>
                    <Button
                        variant="outlined"
                        color="success"
                        startIcon={aiLoading ? <CircularProgress size={18} /> : <Add />}
                        onClick={handleGenerateAI}
                        disabled={aiLoading || informes.length === 0}
                        sx={{ height: 40 }}
                    >
                        Generar informe con IA
                    </Button>
            ...
            <LoaderOverlay open={loading} /></Box>
    )
}
...
const AiReportModal = ({ open, onClose, text }) => {
    return (
        <Backdrop open={open} onClick={onClose} sx={{ zIndex: 1300 }}>
            <Box onClick={(e) => e.stopPropagation()} sx={{ bgcolor: 'background.paper', p: 3, borderRadius: 2, width: 800, maxWidth: '95vw', maxHeight: '85vh', overflow: 'auto' }}>
                <Typography variant="h6" gutterBottom>
                    Informe pedag�gico (IA)
                </Typography>
                <TextareaAutosize
                    minRows={15}
                    style={{ width: '100%', padding: 8, fontSize: 16 }}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 2 }}>
                    <Button variant="outlined" onClick={() => { navigator.clipboard?.writeText(text || ''); }}>
                        Copiar
                    </Button>
